...
The versioned `deploy/blockscout/compose.aichain.yml` override is used with Blockscout's official `geth.yml` template. It binds only the proxy UI to `127.0.0.1:4000`, does not publish either database, uses chain ID `20260818`, and disables trace-dependent/pending-transaction indexers for the current RPC profile. Node 1's RPC stays bound to `127.0.0.1:8545`; a dedicated relay listens only on Docker's private bridge address (`172.17.0.1:18545`) so the explorer containers can reach it. The optional user-operation indexer is excluded because this development node does not expose a normal WebSocket RPC endpoint. Runtime database passwords and `SECRET_KEY_BASE` are generated in the ignored `/opt/aichain/.env.blockscout` file.
