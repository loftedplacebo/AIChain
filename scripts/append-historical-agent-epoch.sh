#!/usr/bin/env bash
# Append a new delegation epoch for the laptop development agent after revocation testing.
set -euo pipefail
...
valid_after="$($FOUNDRY_BIN/cast block latest --field timestamp --rpc-url "$RPC_URL")"; valid_until=$((valid_after + VALIDITY_SECONDS))
