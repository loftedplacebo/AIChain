...
initial_difficulty="${ASERT_INITIAL_DIFFICULTY:-}"
...
python3 - "$base_genesis" "$run_dir/genesis.json" "$address" "$target" "$initial_difficulty" <<'PY'
import json, sys, time
source, destination, address, target_text, difficulty_override = sys.argv[1:]
...
genesis["difficulty"] = hex(int(difficulty_override, 0)) if difficulty_override else hex(difficulties[target])
...
effective_difficulty="$(python3 -c 'import json,sys; print(int(json.load(open(sys.argv[1]))["difficulty"], 16))' "$run_dir/genesis.json")"

cat > "$run_dir/public-run.env" <<EOF
ASERT_TARGET_SECONDS=$target
ASERT_INITIAL_DIFFICULTY=$effective_difficulty
