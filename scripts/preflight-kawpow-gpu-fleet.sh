...
python3 "$project_root/scripts/validate-kawpow-gpu-fleet.py" "$inventory"
mkdir -p "$evidence_dir"

readarray -t fleet < <(python3 - "$inventory" <<'PY'
import json, sys
...
for entry in "${fleet[@]:1}"; do
  ...
  grep -Fq "$expected_gpu" "$output" || { echo "$id GPU does not match expected model" >&2; exit 1; }
  grep -Fq "$compute" "$output" || { echo "$id compute capability does not match" >&2; exit 1; }
done

python3 - "$inventory" "$evidence_dir/manifest.json" <<'PY'
