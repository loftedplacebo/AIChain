...
$actualCommit = (& git -C (Join-Path $spikeDirectory 'cpp-kawpow') rev-parse HEAD).Trim()
if ($actualCommit -ne $expectedCommit) { throw "Unexpected cpp-kawpow commit: $actualCommit" }
...
try {
    & $goExe test ./...
    if ($LASTEXITCODE -ne 0) { throw 'C1 KawPoW verifier spike tests failed.' }
}
finally {
    Pop-Location
}
