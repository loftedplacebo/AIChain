...
type Configuration struct {
	EnforceAmount   bool `json:"enforceAmount"`
	EnforceResource bool `json:"enforceResource"`
}
type Policy struct {
...
type Witness struct {
	...
	Blindings          Blindings     `json:"blindings"`
	ModelCommitment    string        `json:"modelCommitment"`
	ProviderCommitment string        `json:"providerCommitment"`
}
type Metadata struct {
	Issuer                string `json:"issuer"`
	OrganizationID        string `json:"organizationId"`
	AuthorityCommitment   string `json:"authorityCommitment"`
	ClaimedAtEpochSeconds int64  `json:"claimedAtEpochSeconds"`
}
type Public struct {
	AuthorityCommitment   string `json:"authorityCommitment"`
	ClaimedAtEpochSeconds int64  `json:"claimedAtEpochSeconds"`
	CommitmentsRoot       string `json:"commitmentsRoot"`
	Decision              string `json:"decision"`
	Issuer                string `json:"issuer"`
	OrganizationID        string `json:"organizationId"`
	ProgramCommitment     string `json:"programCommitment"`
...
type Document struct {
	...
	PublicMetadata   Metadata `json:"publicMetadata"`
	PrivateWitness   Witness  `json:"privateWitness"`
	ExpectedPublic   Public   `json:"expectedPublic"`
}
...
func bytes32(value string) bool {
...
func sortedUnique(values []string) bool {
	...
	sort.Strings(clone)
	for i := range values {
		if values[i] == "" || values[i] != clone[i] || (i > 0 && values[i] == values[i-1]) {
			return false
		}
	}
	return true
...
func validate(w Witness) error {
	if w.Action.Operation == "" || w.Action.Resource == "" {
		return errors.New("action strings must be non-empty")
	}
	if w.Action.Amount >= 1<<63 || w.Policy.MaxAmount >= 1<<63 {
		return errors.New("amounts must be unsigned 63-bit integers")
	}
	if !sortedUnique(w.Policy.AllowedOperations) || !sortedUnique(w.Policy.AllowedResources) {
		return errors.New("allowlists must be sorted, unique and non-empty")
	}
	for _, value := range []string{w.Blindings.Input, w.Blindings.Output, w.Blindings.Configuration, w.Blindings.Policy, w.ModelCommitment, w.ProviderCommitment} {
		if !bytes32(value) {
			return errors.New("commitments and blindings must be lowercase bytes32")
		}
	}
	return nil
}
...
func Evaluate(w Witness) (map[string]any, error) {
	...
	if !contains(w.Policy.AllowedOperations, w.Action.Operation) {
		reasons = append(reasons, "operation-not-allowed")
	}
	if w.Configuration.EnforceResource && !contains(w.Policy.AllowedResources, w.Action.Resource) {
		reasons = append(reasons, "resource-not-allowed")
	}
	if w.Configuration.EnforceAmount && w.Action.Amount > w.Policy.MaxAmount {
		reasons = append(reasons, "amount-exceeds-limit")
	}
	decision := "allow"
	if len(reasons) > 0 {
		decision = "deny"
...
func blinded(kind string, value any, blinding string) string {
	raw, _ := hex.DecodeString(blinding[2:])
	return hash([]byte(fmt.Sprintf("aichain:zk-witness:%s:%s:", statementVersion, kind)), raw, canonical(value))
}

func DerivePublic(metadata Metadata, witness Witness) (Public, error) {
	if err := validate(witness); err != nil {
		return Public{}, err
	}
	if !bytes32(metadata.OrganizationID) || !bytes32(metadata.AuthorityCommitment) || metadata.ClaimedAtEpochSeconds < 0 {
		return Public{}, errors.New("invalid public metadata")
	}
